  ...
  if (!tsl2591_inited)
  {
    ...
    else
    {
      if (xSemaphoreTake(lvglSemaphore, (TickType_t)10) == pdTRUE)
      {
        ...
        lv_obj_set_size(ui_tsl2591_box, LV_SIZE_CONTENT, (gfx->height() - 88) / 2);
        ...
        lv_obj_set_style_text_font(ui_tsl2591_Label, &lv_font_montserrat_32, LV_PART_MAIN | LV_STATE_DEFAULT);
        ...
        lv_obj_set_size(ui_tsl2591_Chart, gfx->width() - 20, gfx->height() - 40);
        lv_obj_center(ui_tsl2591_Chart);
        lv_obj_set_y(ui_tsl2591_Chart, 10);
